...
static inline States_e _monitor(void){
    ...
    for(uint32_t i=0; i<8; ++i){
        uint16_t bpm = heartbeat_get_bpm();
        LCD_ClearDisplay();
        LCD_Position(0,0);
        LCD_PrintDecUint16(bpm);
        CyDelay(250);
    }
...
static inline Slider_e _check_slider_state(void){
    ...
    if(slider_event.prev_val == NO_TOUCH_ON_SLIDER 
    ...
    //else if up event
    else if(slider_event.prev_val != NO_TOUCH_ON_SLIDER 
            && pos == NO_TOUCH_ON_SLIDER){
        //if slide a left, right, or no match?
        if(slider_event.origin <= SLIDE_RIGHT_START 
                && slider_event.prev_val >= SLIDE_RIGHT_END){
            retVal = SLIDE_RIGHT;
        }else if(slider_event.origin >= SLIDE_LEFT_START 
                && slider_event.prev_val <= SLIDE_LEFT_END){
            retVal = SLIDE_LEFT;
        }else{
            retVal = SLIDE_OFF;
        }
