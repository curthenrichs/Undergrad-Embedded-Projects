/*******************************************************************************
* File Name: I_Heartbeat_DacClk.c
* Version 2.10
*
*  Description:
...
/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_Start
********************************************************************************
*
* Summary:
...
void I_Heartbeat_DacClk_Start(void) 
{
    ...
    I_Heartbeat_DacClk_CLKEN |= I_Heartbeat_DacClk_CLKEN_MASK;
	I_Heartbeat_DacClk_CLKSTBY |= I_Heartbeat_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_Stop
********************************************************************************
*
* Summary:
...
void I_Heartbeat_DacClk_Stop(void) 
{
    ...
    I_Heartbeat_DacClk_CLKEN &= (uint8)(~I_Heartbeat_DacClk_CLKEN_MASK);
	I_Heartbeat_DacClk_CLKSTBY &= (uint8)(~I_Heartbeat_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((I_Heartbeat_DacClk_CLKEN & I_Heartbeat_DacClk_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = I_Heartbeat_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = I_Heartbeat_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* I_Heartbeat_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(I_Heartbeat_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(I_Heartbeat_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = I_Heartbeat_DacClk_GetSourceRegister();
    uint16 oldDivider = I_Heartbeat_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = I_Heartbeat_DacClk_CLKEN & I_Heartbeat_DacClk_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(I_Heartbeat_DacClk_DIV_PTR, clkDivider);
                I_Heartbeat_DacClk_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                I_Heartbeat_DacClk_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(I_Heartbeat_DacClk_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((I_Heartbeat_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(I_Heartbeat_DacClk_DIV_PTR, clkDivider);
				I_Heartbeat_DacClk_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(I_Heartbeat_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    I_Heartbeat_DacClk_MOD_SRC |= modeBitMask & (uint8)I_Heartbeat_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    I_Heartbeat_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(I_Heartbeat_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return I_Heartbeat_DacClk_MOD_SRC & (uint8)(I_Heartbeat_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = I_Heartbeat_DacClk_GetDividerRegister();
    uint8 oldSrc = I_Heartbeat_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        I_Heartbeat_DacClk_MOD_SRC |= CYCLK_SSS;
        I_Heartbeat_DacClk_MOD_SRC =
            (I_Heartbeat_DacClk_MOD_SRC & (uint8)(~I_Heartbeat_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return I_Heartbeat_DacClk_MOD_SRC & I_Heartbeat_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    I_Heartbeat_DacClk_PHASE = clkPhase & I_Heartbeat_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: I_Heartbeat_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
{
    return I_Heartbeat_DacClk_PHASE & I_Heartbeat_DacClk_PHASE_MASK;
}
