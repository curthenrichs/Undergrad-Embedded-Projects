...
/*******************************************************************************
* Function Name: I_HB_DacClk_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_Start(void) 
{
    /* Set the bit to enable the clock. */
    I_HB_DacClk_CLKEN |= I_HB_DacClk_CLKEN_MASK;
	I_HB_DacClk_CLKSTBY |= I_HB_DacClk_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: I_HB_DacClk_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    I_HB_DacClk_CLKEN &= (uint8)(~I_HB_DacClk_CLKEN_MASK);
	I_HB_DacClk_CLKSTBY &= (uint8)(~I_HB_DacClk_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: I_HB_DacClk_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_StopBlock(void) 
{
    if ((I_HB_DacClk_CLKEN & I_HB_DacClk_CLKEN_MASK) != 0u)
    {
...
#if defined(I_HB_DacClk__CFG3)
        CLK_DIST_AMASK = I_HB_DacClk_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = I_HB_DacClk_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* I_HB_DacClk__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(I_HB_DacClk_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(I_HB_DacClk_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: I_HB_DacClk_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        I_HB_DacClk_CLKSTBY &= (uint8)(~I_HB_DacClk_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: I_HB_DacClk_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = I_HB_DacClk_GetSourceRegister();
    uint16 oldDivider = I_HB_DacClk_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = I_HB_DacClk_CLKEN & I_HB_DacClk_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((I_HB_DacClk_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: I_HB_DacClk_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 I_HB_DacClk_GetDividerRegister(void) 
{
    return CY_GET_REG16(I_HB_DacClk_DIV_PTR);
}


/*******************************************************************************
* Function Name: I_HB_DacClk_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_SetModeRegister(uint8 modeBitMask) 
{
    I_HB_DacClk_MOD_SRC |= modeBitMask & (uint8)I_HB_DacClk_MODE_MASK;
}


/*******************************************************************************
* Function Name: I_HB_DacClk_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_ClearModeRegister(uint8 modeBitMask) 
{
    I_HB_DacClk_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(I_HB_DacClk_MODE_MASK));
}


/*******************************************************************************
* Function Name: I_HB_DacClk_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 I_HB_DacClk_GetModeRegister(void) 
{
    return I_HB_DacClk_MOD_SRC & (uint8)(I_HB_DacClk_MODE_MASK);
}


/*******************************************************************************
* Function Name: I_HB_DacClk_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = I_HB_DacClk_GetDividerRegister();
    uint8 oldSrc = I_HB_DacClk_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        I_HB_DacClk_MOD_SRC |= CYCLK_SSS;
        I_HB_DacClk_MOD_SRC =
            (I_HB_DacClk_MOD_SRC & (uint8)(~I_HB_DacClk_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: I_HB_DacClk_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 I_HB_DacClk_GetSourceRegister(void) 
{
    return I_HB_DacClk_MOD_SRC & I_HB_DacClk_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: I_HB_DacClk_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void I_HB_DacClk_SetPhaseRegister(uint8 clkPhase) 
{
    I_HB_DacClk_PHASE = clkPhase & I_HB_DacClk_PHASE_MASK;
}


/*******************************************************************************
* Function Name: I_HB_DacClk_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 I_HB_DacClk_GetPhaseRegister(void) 
{
    return I_HB_DacClk_PHASE & I_HB_DacClk_PHASE_MASK;
}
