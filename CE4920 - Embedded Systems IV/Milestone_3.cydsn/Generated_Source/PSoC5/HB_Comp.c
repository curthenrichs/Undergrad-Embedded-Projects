...
uint8 HB_Comp_initVar = 0u;

/* Internal functions definitoin */
static void HB_Comp_trimAdjust(uint8 nibble) ;

/* static HB_Comp_backupStruct  HB_Comp_backup; */
#if (CY_PSOC5A)
    static HB_Comp_LOWPOWER_BACKUP_STRUCT  HB_Comp_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 HB_Comp_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: HB_Comp_Init
********************************************************************************
*
* Summary:
...
void HB_Comp_Init(void) 
{
    ...
    HB_Comp_SetSpeed(HB_Comp_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( HB_Comp_DEFAULT_HYSTERESIS == 0u )
        HB_Comp_CR |= HB_Comp_HYST_OFF;
    #else
        HB_Comp_CR &= (uint8)(~HB_Comp_HYST_OFF);
    #endif /* HB_Comp_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( HB_Comp_DEFAULT_PWRDWN_OVRD == 0u )
            HB_Comp_CR &= (uint8)(~HB_Comp_PWRDWN_OVRD);
        #else 
            HB_Comp_CR |= HB_Comp_PWRDWN_OVRD;
        #endif /* HB_Comp_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    HB_Comp_CR |= HB_Comp_MX_AO;

    /* Set default sync */
    HB_Comp_CLK &= (uint8)(~HB_Comp_SYNCCLK_MASK);
    #if ( HB_Comp_DEFAULT_BYPASS_SYNC == 0u )
        HB_Comp_CLK |= HB_Comp_SYNC_CLK_EN;
    #else
        HB_Comp_CLK |= HB_Comp_BYPASS_SYNC;
    #endif /* HB_Comp_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: HB_Comp_Enable
********************************************************************************
*
* Summary:
...
{
    HB_Comp_PWRMGR |= HB_Comp_ACT_PWR_EN;
    HB_Comp_STBY_PWRMGR |= HB_Comp_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(HB_Comp_restoreReg == 1u)
        {
            HB_Comp_CR = HB_Comp_lowPowerBackup.compCRReg;

            /* Clear the flag */
            HB_Comp_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  HB_Comp_initVar: Is modified when this function is called for the 
...
void HB_Comp_Stop(void) 
{
    ...
    HB_Comp_PWRMGR &= (uint8)(~HB_Comp_ACT_PWR_EN);
    HB_Comp_STBY_PWRMGR &= (uint8)(~HB_Comp_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        HB_Comp_restoreReg = 1u;

        /* Save the control register before clearing it */
        HB_Comp_lowPowerBackup.compCRReg = HB_Comp_CR;
        HB_Comp_CR = HB_Comp_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: HB_Comp_SetSpeed
********************************************************************************
*
* Summary:
...
void HB_Comp_SetSpeed(uint8 speed) 
{
    ...
    HB_Comp_CR = (HB_Comp_CR & (uint8)(~HB_Comp_PWR_MODE_MASK)) |
                           (speed & HB_Comp_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == HB_Comp_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            HB_Comp_TR = HB_Comp_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            HB_Comp_TR0 = HB_Comp_HS_TRIM_TR0;
            HB_Comp_TR1 = HB_Comp_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            HB_Comp_TR = HB_Comp_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            HB_Comp_TR0 = HB_Comp_LS_TRIM_TR0;
            HB_Comp_TR1 = HB_Comp_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: HB_Comp_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( HB_Comp_WRK & HB_Comp_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: HB_Comp_trimAdjust
********************************************************************************
*
* Summary:
...
static void HB_Comp_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = HB_Comp_WRK & HB_Comp_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                HB_Comp_TR |= HB_Comp_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                HB_Comp_TR0 |= HB_Comp_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                HB_Comp_TR |= HB_Comp_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                HB_Comp_TR1 |= HB_Comp_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((HB_Comp_WRK & HB_Comp_CMP_OUT_MASK) != cmpState)
        {
...
uint16 HB_Comp_ZeroCal(void) 
{
    ...
    HB_Comp_SW0 = 0x00u;
    HB_Comp_SW2 = 0x00u;
    HB_Comp_SW3 = tmpSW3 & (uint8)(~HB_Comp_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = HB_Comp_CR;
    HB_Comp_CR |= (HB_Comp_CAL_ON | HB_Comp_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        HB_Comp_TR = HB_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        HB_Comp_TR0 = HB_Comp_DEFAULT_CMP_TRIM;
        HB_Comp_TR1 = HB_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (HB_Comp_CR & HB_Comp_PWR_MODE_MASK) == HB_Comp_PWR_MODE_FAST)
    {
        HB_Comp_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        HB_Comp_trimAdjust(1u);
		HB_Comp_trimAdjust(0u);
    }
    ...
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) HB_Comp_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)HB_Comp_TR1 << 8u) | (HB_Comp_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: HB_Comp_LoadTrim
********************************************************************************
*
* Summary:
...
void HB_Comp_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: HB_Comp_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: HB_Comp_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
