...
/*******************************************************************************
* Function Name: CLK_2KHZ_1_Start
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_Start(void) 
{
    /* Set the bit to enable the clock. */
    CLK_2KHZ_1_CLKEN |= CLK_2KHZ_1_CLKEN_MASK;
	CLK_2KHZ_1_CLKSTBY |= CLK_2KHZ_1_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLK_2KHZ_1_Stop
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_Stop(void) 
{
    /* Clear the bit to disable the clock. */
    CLK_2KHZ_1_CLKEN &= (uint8)(~CLK_2KHZ_1_CLKEN_MASK);
	CLK_2KHZ_1_CLKSTBY &= (uint8)(~CLK_2KHZ_1_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_2KHZ_1_StopBlock
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_StopBlock(void) 
{
    if ((CLK_2KHZ_1_CLKEN & CLK_2KHZ_1_CLKEN_MASK) != 0u)
    {
...
#if defined(CLK_2KHZ_1__CFG3)
        CLK_DIST_AMASK = CLK_2KHZ_1_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_2KHZ_1_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_2KHZ_1__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_2KHZ_1_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_2KHZ_1_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_2KHZ_1_StandbyPower
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_StandbyPower(uint8 state) 
{
    if(state == 0u)
    {
        CLK_2KHZ_1_CLKSTBY &= (uint8)(~CLK_2KHZ_1_CLKSTBY_MASK);
...
/*******************************************************************************
* Function Name: CLK_2KHZ_1_SetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_SetDividerRegister(uint16 clkDivider, uint8 restart)
                                
{
    uint8 enabled;

    uint8 currSrc = CLK_2KHZ_1_GetSourceRegister();
    uint16 oldDivider = CLK_2KHZ_1_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_2KHZ_1_CLKEN & CLK_2KHZ_1_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_2KHZ_1_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLK_2KHZ_1_GetDividerRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint16 CLK_2KHZ_1_GetDividerRegister(void) 
{
    return CY_GET_REG16(CLK_2KHZ_1_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_2KHZ_1_SetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_SetModeRegister(uint8 modeBitMask) 
{
    CLK_2KHZ_1_MOD_SRC |= modeBitMask & (uint8)CLK_2KHZ_1_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_2KHZ_1_ClearModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_ClearModeRegister(uint8 modeBitMask) 
{
    CLK_2KHZ_1_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_2KHZ_1_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_2KHZ_1_GetModeRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CLK_2KHZ_1_GetModeRegister(void) 
{
    return CLK_2KHZ_1_MOD_SRC & (uint8)(CLK_2KHZ_1_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_2KHZ_1_SetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_SetSourceRegister(uint8 clkSource) 
{
    uint16 currDiv = CLK_2KHZ_1_GetDividerRegister();
    uint8 oldSrc = CLK_2KHZ_1_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_2KHZ_1_MOD_SRC |= CYCLK_SSS;
        CLK_2KHZ_1_MOD_SRC =
            (CLK_2KHZ_1_MOD_SRC & (uint8)(~CLK_2KHZ_1_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_2KHZ_1_GetSourceRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CLK_2KHZ_1_GetSourceRegister(void) 
{
    return CLK_2KHZ_1_MOD_SRC & CLK_2KHZ_1_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_2KHZ_1_SetPhaseRegister
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
void CLK_2KHZ_1_SetPhaseRegister(uint8 clkPhase) 
{
    CLK_2KHZ_1_PHASE = clkPhase & CLK_2KHZ_1_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_2KHZ_1_GetPhase
********************************************************************************
*
* Summary:
...
*
*******************************************************************************/
uint8 CLK_2KHZ_1_GetPhaseRegister(void) 
{
    return CLK_2KHZ_1_PHASE & CLK_2KHZ_1_PHASE_MASK;
}
