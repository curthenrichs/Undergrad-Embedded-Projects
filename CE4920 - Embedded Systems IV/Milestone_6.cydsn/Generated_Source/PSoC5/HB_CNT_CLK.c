...
/*******************************************************************************
* Function Name: HB_CNT_CLK_Start
********************************************************************************
*
* Summary:
...
void HB_CNT_CLK_Start(void) 
{
    ...
    HB_CNT_CLK_CLKEN |= HB_CNT_CLK_CLKEN_MASK;
	HB_CNT_CLK_CLKSTBY |= HB_CNT_CLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: HB_CNT_CLK_Stop
********************************************************************************
*
* Summary:
...
void HB_CNT_CLK_Stop(void) 
{
    ...
    HB_CNT_CLK_CLKEN &= (uint8)(~HB_CNT_CLK_CLKEN_MASK);
	HB_CNT_CLK_CLKSTBY &= (uint8)(~HB_CNT_CLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: HB_CNT_CLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((HB_CNT_CLK_CLKEN & HB_CNT_CLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = HB_CNT_CLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = HB_CNT_CLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* HB_CNT_CLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(HB_CNT_CLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(HB_CNT_CLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: HB_CNT_CLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: HB_CNT_CLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = HB_CNT_CLK_GetSourceRegister();
    uint16 oldDivider = HB_CNT_CLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = HB_CNT_CLK_CLKEN & HB_CNT_CLK_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((HB_CNT_CLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: HB_CNT_CLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(HB_CNT_CLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: HB_CNT_CLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    HB_CNT_CLK_MOD_SRC |= modeBitMask & (uint8)HB_CNT_CLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: HB_CNT_CLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    HB_CNT_CLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(HB_CNT_CLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: HB_CNT_CLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return HB_CNT_CLK_MOD_SRC & (uint8)(HB_CNT_CLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: HB_CNT_CLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = HB_CNT_CLK_GetDividerRegister();
    uint8 oldSrc = HB_CNT_CLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        HB_CNT_CLK_MOD_SRC |= CYCLK_SSS;
        HB_CNT_CLK_MOD_SRC =
            (HB_CNT_CLK_MOD_SRC & (uint8)(~HB_CNT_CLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: HB_CNT_CLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return HB_CNT_CLK_MOD_SRC & HB_CNT_CLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: HB_CNT_CLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    HB_CNT_CLK_PHASE = clkPhase & HB_CNT_CLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: HB_CNT_CLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return HB_CNT_CLK_PHASE & HB_CNT_CLK_PHASE_MASK;
}
