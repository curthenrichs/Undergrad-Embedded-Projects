...
/*******************************************************************************
* Function Name: CLK_100KHz_2_Start
********************************************************************************
*
* Summary:
...
void CLK_100KHz_2_Start(void) 
{
    ...
    CLK_100KHz_2_CLKEN |= CLK_100KHz_2_CLKEN_MASK;
	CLK_100KHz_2_CLKSTBY |= CLK_100KHz_2_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: CLK_100KHz_2_Stop
********************************************************************************
*
* Summary:
...
void CLK_100KHz_2_Stop(void) 
{
    ...
    CLK_100KHz_2_CLKEN &= (uint8)(~CLK_100KHz_2_CLKEN_MASK);
	CLK_100KHz_2_CLKSTBY &= (uint8)(~CLK_100KHz_2_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: CLK_100KHz_2_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((CLK_100KHz_2_CLKEN & CLK_100KHz_2_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = CLK_100KHz_2_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = CLK_100KHz_2_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* CLK_100KHz_2__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(CLK_100KHz_2_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(CLK_100KHz_2_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: CLK_100KHz_2_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: CLK_100KHz_2_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = CLK_100KHz_2_GetSourceRegister();
    uint16 oldDivider = CLK_100KHz_2_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = CLK_100KHz_2_CLKEN & CLK_100KHz_2_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(CLK_100KHz_2_DIV_PTR, clkDivider);
                CLK_100KHz_2_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                CLK_100KHz_2_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(CLK_100KHz_2_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((CLK_100KHz_2_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: CLK_100KHz_2_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(CLK_100KHz_2_DIV_PTR);
}


/*******************************************************************************
* Function Name: CLK_100KHz_2_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_100KHz_2_MOD_SRC |= modeBitMask & (uint8)CLK_100KHz_2_MODE_MASK;
}


/*******************************************************************************
* Function Name: CLK_100KHz_2_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    CLK_100KHz_2_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(CLK_100KHz_2_MODE_MASK));
}


/*******************************************************************************
* Function Name: CLK_100KHz_2_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_100KHz_2_MOD_SRC & (uint8)(CLK_100KHz_2_MODE_MASK);
}


/*******************************************************************************
* Function Name: CLK_100KHz_2_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = CLK_100KHz_2_GetDividerRegister();
    uint8 oldSrc = CLK_100KHz_2_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        CLK_100KHz_2_MOD_SRC |= CYCLK_SSS;
        CLK_100KHz_2_MOD_SRC =
            (CLK_100KHz_2_MOD_SRC & (uint8)(~CLK_100KHz_2_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: CLK_100KHz_2_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return CLK_100KHz_2_MOD_SRC & CLK_100KHz_2_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: CLK_100KHz_2_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    CLK_100KHz_2_PHASE = clkPhase & CLK_100KHz_2_PHASE_MASK;
}


/*******************************************************************************
* Function Name: CLK_100KHz_2_GetPhase
********************************************************************************
*
* Summary:
...
{
    return CLK_100KHz_2_PHASE & CLK_100KHz_2_PHASE_MASK;
}
